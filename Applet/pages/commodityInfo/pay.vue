        ...
        <view v-if="!uLoadingPageShow" class="main">
            <view class="item-address" @click="addressShowClick" v-if="info" style="margin-top: 15px;">
                ...
                <view class="content">
                    ...
                    <text
                        class="address">{{address.province + (address.city == address.province ? '' : address.city) + address.county + address.address_detail}}</text>
                </view>
            </view>
            ...
            <view class="show-item">
                ...
                <text class="value" @click="couponShowClick"
                    :style="{color:couponList.length == 0 ? '#333' : 'red'}">{{couponList.length == 0 ? '暂无可用优惠券' : coupon.name}}</text>
            </view>
            <view class="show-item">
            ...
            <u-popup :show="couponShow" @close="couponShowClose" mode="bottom" :round="10" @click.stop="1 + 1"
                ...
                <view class="select_sp">
                    ...
                    <u-empty v-if="couponList.length <= 0" icon="/static/nono.png" text="还没有优惠券"
                        marginTop="100"></u-empty>

                    <view class="address_list">
                        <view v-for="(item,index) in couponList" class="co-item" :key="index"
                            @click="couponChange(item)" :style="{background:item.min > (Number((Number(specification.price) * count).toFixed(2)) + Number(Number(info
                    .postage).toFixed(2)))  ? '#f3f3f3' : 'rgba(71,200,172,0.1)'  }">
                            <text class="name">{{item.name}}</text>
                            <text class="time"
                                v-if="item.name != '不使用优惠券'">有效期：{{item.start_time}}至{{item.end_time}}</text>
