    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 用户收货地址
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            <div class="handle-box" style="position: relative;">
                ...
                <el-select v-model="query.type" placeholder="选择搜索条件" @change="selectTypeChange" class="handle-select mr10">
            ...
            <el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header">
                ...
                <el-table-column prop="province" label="省" width="140"></el-table-column>
                <el-table-column prop="city" label="市" width="140" :show-overflow-tooltip="true"></el-table-column>
                <el-table-column prop="county" label="区" width="140" :show-overflow-tooltip="true"></el-table-column>
                <el-table-column label="详细地址" min-width="170" :show-overflow-tooltip="true">
                    <template slot-scope="scope">
                        <span style="cursor: pointer;" @click="showAddress(scope.row)">{{ scope.row.address_detail }}</span>
                    </template>
                </el-table-column>
                <el-table-column label="用户默认地址" align="center" width="120">
                    <template slot-scope="scope">
                        <el-tag :type="scope.row.is_default == '1' ? 'success' : 'info'">{{ scope.row.is_default == '1' ?
                            '默认' : '非默认' }}</el-tag>
                    </template>
                </el-table-column>
...
<script>
import { getUserAddress, userAddressDelete, getUserList } from '../../api/index'
export default {
    name: 'basetable',
    data() {
        return {
    ...
    methods: {
        // 获取数据
        getData() {
            getUserAddress(this.query).then(res => {
                if (res.code == 200) {
        ...
        handleDelete(index, row) {
            ...
            this.$confirm(`此操作将永久删除该数据, 是否继续?`, '提示', {
                ...
                userAddressDelete(row.id).then(res => {
                    if (res.code == 200) {
                        this.$message({
                            type: 'success',
                            message: '操作成功!'
        ...
        showAddress(obj) {
            console.log(obj);
            let str = obj.province + (obj.province == obj.city ? '' : obj.city) + obj.county + obj.address_detail
            this.$alert(str, '完整地址', {
                confirmButtonText: '确定'
            });
        },
